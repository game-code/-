...
export default class EffectManager extends cc.Component {
    ...
    useShader() {
        ...
        let gltext1Normal;
        if (this.shaderNormalSpr) {
            let texture1Normal = this.shaderNormalSpr.getTexture();
            gltext1Normal = texture1Normal._glID;
            if (texture1Normal) {
                if (cc.sys.isNative) {
                }
                else {
                    // cc.gl.deleteTexture2DN(2, texture1Normal);
                    cc.gl.bindTexture2DN(2, texture1Normal);
                }
            }
        }

        let gltext1Diffuse;
        if (this.shaderDiffuseSpr) {
            let texture1Diffuse = this.shaderDiffuseSpr.getTexture();
            gltext1Diffuse = texture1Diffuse._glID;
            if (texture1Diffuse) {
                if (cc.sys.isNative) {
                }
                else {
                    // cc.gl.deleteTexture2DN(3, texture1Diffuse);
                    cc.gl.bindTexture2DN(3, texture1Diffuse);
                }
            }
        }
        ...
        if (cc.sys.isNative) {
            ...
            glProgram_state.setUniformTexture("texturediffuse", gltext1);
            ...
            this.program.setUniformLocationWith1i(text1normal, 2);
            this.program.setUniformLocationWith1i(text1diffuse, 3);
            for (let i = 0; i < this.posArr.length; ++i) {
    ...
    setProgram(node: cc.SGNode, program: any) {
        ...
        // cc.gl.bindTexture2DN(1, null);
        // cc.gl.bindTexture2DN(2, null);
        // cc.gl.bindTexture2DN(3, null);
    }
